...
\author{\IEEEauthorblockN{Blinded for review\vspace{-30pt} } }
...
\begin{quote}
\centering {\em Today the computer is just as important a tool for chemists as the test tube~\cite{nobel_2013}.\vspace{-2pt}}
\end{quote}
...
Only in the case of {\em duration}, the CSc median were statistically different and less than the SE medians, measured by Scott-Knott\footnote{
...
This procedure has been endorsed in the recent SE literature~\cite{mittas2013ranking,arcuri2011practical}.}. All the other indicators show that CSc projects are just as active (or even more active) than SE projects. 
...
Github stores data about code and the comments seen during code reviews and pull requests. While this is useful for assessing most of the beliefs of~\Cref{tab:characteristics}, it does mean that five of the thirteen beliefs, summarized by~\citet{johan18_secs}, cannot be explored by this paper:
...
% (B) the nature of scientific challenges,
...
\noindent \textit{\underline{Observed:}} Specifically, this belief analysis can only be completed within CSc projects as scientific activities are not typical for SE projects. Across the enhancement type commits from the sample from~\Cref{fig:SE_activities}, 31\% of the enhancement commits focus on the core science while 35\% of the enhancement commits focus on the code quality. The absolute difference between the two types of enhancement activities is small (4\%). Moreover, from~\Cref{fig:belief1}, the rate of commits of engineering (square) and scientific (triangle) enhancement activities are observed to grow synchronously across the project lifetime. 
...
\footnotesize
% \caption{First two columns are TP/P rate after CLA and Easy while the last two columns are the median ratios of found actual TDs over estimated TDs (TDs') per iteration of the active learning strategy in the second step of Jitterbug  and DebtFree. }
\caption{Given $N$ projects, the EASY pattern recognizer learned patterns on $N-1$ projects to recognize technical debt on $N$-th project.  EASY's performance for precision is 100\% for SE projects while only 13.5\% for CSc projects on average,}
\vspace{-5pt}
\resizebox{\linewidth}{!}{\minipage{0.45\linewidth}%
\centering
\subcaption{EASY on SE datasets \vspace{5pt}}

\begin{tabular}{l|c}
\setlength\tabcolsep{4.5pt} 
% & \multicolumn{2}{c}{TP rate} \\ 
% \hline
Datasets & Precision    \\ \hline
Columba 	&	99	 \\ 
Apache Ant 	&	99 	 \\
SQuirrel 	&	100	 \\
JMeter 	&	100	 \\
...
\end{tabular} 
\endminipage
\hspace{10pt}\minipage{0.45\linewidth}%
\centering
 \subcaption{EASY on CSc datasets \vspace{5pt}}
 
 \begin{tabular}{l|c}
\setlength\tabcolsep{4.5pt} 
% & \multicolumn{3}{c}{TDs/TDs' rate} \\ 
% \hline
Datasets & Precision \\ \hline 
MDANALYSIS 	&	0 	\\
LAMMPS 	&	 0 	 \\
...
\resizebox{1.05\linewidth}{!}{
 \hspace{-10mm}
 \includegraphics[width=0.525\linewidth, height=1.8in]{img/SE_heroes_commits.png}
 \hspace{-15mm}
 \includegraphics[width=0.525\linewidth, height=1.8in]{img/CS_heroes_commit.png}}
 
%  \resizebox{1.05\linewidth}{!}{
%  \hspace{-10mm}
%  \includegraphics[width=0.525\linewidth, height=1.8in]{img/SE_heroes_social.png}
%  \hspace{-10mm}
%  \includegraphics[width=0.525\linewidth, height=1.8in]{img/CS_heroes_social.png}}

\vspace{-10pt}
%  \caption{Percentage of new bugs from developer social interaction (issues conversation), made by hero and non-hero developers from SE (left) and CSc (right) projects.
%  Each x-axis of that figure is one project. The hero and non-hero defect introduction rate (defects per social interaction) is the ratio of the blue to red values
...
\mbox{$\kappa = 0.78$~\cite{irr_kappa}}.~\Cref{tbl:testing} shows that scientific testing is the largest type of Testing commits sample (47\%). 
...
\end{tabular}%}
\vspace{-10pt}
\end{wraptable}To show that the CSc assurance level is ``as much or more difficult'' than in SE,
...
Note that 15\% is 2.5 times larger than 6\%.~\Cref{tbl:testing_travis} summarizes the median and interquartile range of testing rigor for both CSc and SE projects. Across all four metrics, CSc projects are reported to test more rigorously than SE projects. However, by the statistical Scott-Knott test, the difference in testing rigor is not distinguishable between CSc and SE. 
...
%That said, it is certainly true that another sample of different projects would make different conclusions.
